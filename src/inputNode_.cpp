...
class InputManager
{
    ...
    private:
        ...
        void seg_h_PCCb_(const depth_clustering::PointCloudArrayConstPtr &seg_h_cloudMsg);

        // int FindPC(const ros::Time stamp, CloudT::Ptr h_cloud, CloudT::Ptr v_cloud);
        int FindPC(const ros::Time stamp, CloudT::Ptr h_cloud, CloudT::Ptr v_cloud, depth_clustering::PointCloudArray::Ptr seg_h_cloud);
...
InputManager::InputManager(ros::NodeHandle nh) : nh_(nh), tf_listener_{tf_buffer_}
{
    ...
    // (6) new-ACRE-long
    nh_.param<float>("min_odom_distance", minOdomDistance_, 0.1); //0.05
    nh_.param<float>("max_time_difference", maxTimeDifference_, 0.02); //0.05
    ...
    nh_.param<std::string>("seg_h_cloud_topic", seg_h_cloud_topic_, "/depth_clustering/object_segments_cloud_array");
    ...
    seg_h_PCSub_ = nh_.subscribe(seg_h_cloud_topic_, 10, &InputManager::seg_h_PCCb_, this);
...
bool InputManager::Run()
{
    ...
    if (odomQueue_.empty() || h_pcQueue_.empty() || v_pcQueue_.empty() || seg_h_pcQueue_.empty()){
        return false;
    }
...
{   
    ...
    if (h_pcQueue_.empty() || v_pcQueue_.empty() || seg_h_pcQueue_.empty()){
     ...
     while (!h_pcQueue_.empty() && !v_pcQueue_.empty() && !seg_h_pcQueue_.empty()){
        // ROS_DEBUG_STREAM("SIZE: " << h_pcQueue_.size() << " " << seg_h_pcQueue_.size() << " " << v_pcQueue_.size());
        if (h_pcQueue_.front()->header.stamp.toSec() < stamp.toSec() - maxTimeDifference_){
            //ROS_DEBUG_STREAM("H_PCLOUD MSG TOO OLD " << h_pcQueue_.front()->header.stamp.toSec() << " " << stamp.toSec());
            h_pcQueue_.pop();
            // seg_h_pcQueue_.pop();
        }
        else if (h_pcQueue_.front()->header.stamp.toSec() > stamp.toSec() + maxTimeDifference_){
            //ROS_DEBUG_STREAM("H_PCLOUD MSG TOO NEW " << h_pcQueue_.front()->header.stamp.toSec() << " " << stamp.toSec());
            return CLOUD_TOO_NEW;
        }
        else{
            if (v_pcQueue_.front()->header.stamp.toSec() < stamp.toSec() - maxTimeDifference_){
                //ROS_DEBUG_STREAM("V_PCLOUD MSG TOO OLD " << v_pcQueue_.front()->header.stamp.toSec() << " " << stamp.toSec());
                v_pcQueue_.pop();
            }
            else if (v_pcQueue_.front()->header.stamp.toSec() > stamp.toSec() + maxTimeDifference_){
                //ROS_DEBUG_STREAM("V_PCLOUD MSG TOO NEW " << v_pcQueue_.front()->header.stamp.toSec() << " " << stamp.toSec());
                return CLOUD_TOO_NEW;
            }
            else{   
        ...
        // else{
        //     if (v_pcQueue_.front()->cloud_array[0].header.stamp.toSec() < stamp.toSec() - maxTimeDifference_){
        //         // ROS_DEBUG_STREAM("V_PCLOUD MSG TOO OLD");
        //         v_pcQueue_.pop();
        //     }
        //     else if (v_pcQueue_.front()->cloud_array[0].header.stamp.toSec() > stamp.toSec() + maxTimeDifference_){
        //         // ROS_DEBUG_STREAM("V_PCLOUD MSG TOO NEW");
        //         return CLOUD_TOO_NEW;
        //     }
            // else{
            //     ROS_DEBUG_STREAM("Calling PAGSLAM:\n h_cloud: " << h_pcQueue_.front()->header.stamp.toSec() << "\n v_cloud: " << v_pcQueue_.front()->header.stamp.toSec() << "\n odom:" << stamp.toSec());
            //     // ROS_DEBUG_STREAM("Calling PAGSLAM:\n h_cloud: " << h_pcQueue_.front()->header.stamp.toSec() << "\n v_cloud: " << v_pcQueue_.front()->cloud_array[0].header.stamp.toSec() << "\n odom:" << stamp.toSec());
